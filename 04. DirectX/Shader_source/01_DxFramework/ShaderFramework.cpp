...
#define ASPECT_RATIO (WIN_WIDTH/(float)WIN_HEIGHT ) // 화연의 종횡비
...
void RenderScene()
{
	...
	D3DXCOLOR color(0.2f, 0.5f, 0.5f, 1.0f);
	g_pShader->SetValue("gColor", &color, sizeof(D3DXVECTOR4));
...
bool LoadAssets()
{
	...
	// 셰이더 로딩
	// g_pShader = LoadShader("ColorShader.fx");
...
void Cleanup()
{
	...
	// 텍스처를 release 한다.
	if (g_pShaderTexture)
		g_pShaderTexture->Release();
	g_pShaderTexture = NULL;
