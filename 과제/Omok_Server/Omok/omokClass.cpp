...
bool Omok::CheckOmok_class()
{
	return CheckOmok(map, colorMap);
}

void Omok::SetPlayerOmok(int saveVal[], int saveNum)
{
	map[saveVal[1]][saveVal[0]] = saveNum;
	colorMap[saveVal[1]][saveVal[0]] = saveNum;
	playerColor.push_back(saveNum);
}

void Omok::ErasePlayerOmok(int saveVal[])
{
	playerColor.pop_back();
	colorMap[saveVal[0]][saveVal[1]] = 0;
}
